...
# Define the leave-one-out cross-validation function for Lasso regression
leave_one_out_cv_lasso <- function(data_df) {
  ...
  for (i in 1:nrow(data_df)) {
    ...
    predictions[i] <- predict(lasso_model, s = best_lambda, newx = x_test)
  }
...
predictions <- leave_one_out_cv_lasso(data_df)

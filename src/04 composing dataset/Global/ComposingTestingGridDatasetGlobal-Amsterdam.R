...
tifs5_dir <- normalizePath(file.path(parent_directory, config$tifs$tifs5), winslash = "/")
rlist <- list.files(path = tifs5_dir, pattern = '.tif$', ignore.case = TRUE, full.names = FALSE)

# Assign each raster to a variable named after the file (without extension)
for (i in rlist) {
...
for (i in seq_along(predictors)) {
  ...
  centroid_predictors[[paste0('pred_', prednames[[i]])]] <- centroid_pred
}
...
# EXPORT OPTIONS
sf::st_write(Cen100_GlobalPredictors, dsn = file.path(out_location_dir, 'Cen100_GlobalPredictors-Amsterdam.gpkg'), driver = "GPKG")

...
grid <- readOGR('/TooBigData/grid100Hamburg.gpkg') #obtained via AreaOfInterest-HH.R
...
rastlist <- list.files(path = "/data/Hamburg/TIFFS/1", pattern='.TIF$', all.files=TRUE, full.names=FALSE)

#define current working directory
setwd("/data/Hamburg/TIFFS/1")
...
dis <- readOGR('/TooBigData/dissolve_BuildingDensity_Hamburg.gpkg')
...
traffic <- readOGR(traffic_volume_study_area_dir ) #obtained via AssignTrafficVolumeToRoads.R
...
## == export options == ##
#shp - centroids
#sf::st_write(Cen100_GlobalPredictors, dsn="/TooBigData/Cen100_GlobalPredictors-HH.gpkg", driver = "GPKG")
#shp - grid format
sf::st_write(Grid100_GlobalPredictors, dsn="/TooBigData/SpatialPredictionPatterns/Grid100_GlobalPredictors-HH.gpkg", driver = "GPKG")
...
#csv
write.csv(Cen100_GlobalPredictors_wgs, '/TooBigData/SpatialPredictionPatterns/grid100_GlobalPredictors-HH.csv')

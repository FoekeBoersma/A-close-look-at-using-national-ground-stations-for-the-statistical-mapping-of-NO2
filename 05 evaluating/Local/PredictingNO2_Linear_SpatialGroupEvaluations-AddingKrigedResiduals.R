...
# 
# #create different datasets, based on the spatial character - select on recently created column "spachar" which describes the spatial group
...
data_sp <- as(data_32, "Spatial") #spatial points dataframe as input for spatial operations (e.g. kriging)
...
leave_one_out_cv <- function(data_df) {
  ...
  for (i in 1:nrow(data_df)) {
    ...
    crs(train_data_sf) <- "+proj=utm +zone=32 +datum=NAD83 +units=m +no_defs"

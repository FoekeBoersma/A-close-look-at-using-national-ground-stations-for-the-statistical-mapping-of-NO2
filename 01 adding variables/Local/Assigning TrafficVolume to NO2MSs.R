...
ms_stations <- read.csv(file = 'C:/Users/foeke/OneDrive/Documenten/april onwards/2022/Initial dataset/ForModelling/LocalModels/LocalModelData.csv', sep= ",")
print(ms_stations)
...
print(ms_stations_nl_alldata)

## == CREATE BUFFERS AROUND NO2 MEASUREMENT STATIONS == ##
##  First project data into a planar coordinate system 


ms_stations_nl_alldata$M_id <- seq(1, nrow(ms_stations_nl_alldata)) #import for later data processing (@ ## == CALCULATE AVERAGE TRAFFIC FOR EACH BUFFER == ##)
ms_stations_nl<- ms_stations_nl_alldata %>% dplyr::select(geometry,M_id)

#export option
sf::st_write(ms_stations_nl, dsn='C:/Users/foeke/OneDrive/Documenten/april onwards/2022/Initial dataset/ForModelling/LocalModels/TrafficLocal',layer='ms_stations_nl', driver = "ESRI Shapefile")

ms_stations_utm <- st_transform(ms_stations_nl, crs=crs_32)
...
for(i in bufs){
  ...
  sf::st_write(buf_i, dsn='C:/Users/foeke/OneDrive/Documenten/april onwards/2022/Initial dataset/ForModelling/LocalModels/TrafficLocal',layer=layer, driver = "ESRI Shapefile")
...
roads_utm <- st_transform(roads_NL, crs=crs_32)
st_crs(roads_utm)
...
while(j <= length(buffer_vars)){
  ...
  print(layer_clip)
  sf::st_write(clip, dsn="C:/Users/foeke/OneDrive/Documenten/april onwards/2022/Initial dataset/ForModelling/LocalModels/TrafficLocal", layer=layer_clip, driver = "ESRI Shapefile")
  ...
  print(layer_int)
  sf::st_write(int_j, dsn="C:/Users/foeke/OneDrive/Documenten/april onwards/2022/Initial dataset/ForModelling/LocalModels/TrafficLocal", layer=layer_int, driver = "ESRI Shapefile")
  ...
  #export option
  sf::st_write(dis, dsn="C:/Users/foeke/OneDrive/Documenten/april onwards/2022/Initial dataset/ForModelling/LocalModels/TrafficLocal", layer=layer_dis, driver = "ESRI Shapefile")
  ...
  #export option
  sf::st_write(merge, dsn="C:/Users/foeke/OneDrive/Documenten/april onwards/2022/Initial dataset/ForModelling/LocalModels/TrafficLocal", layer=layer_mer, driver = "ESRI Shapefile")
...
sf::st_write(ms_traffic_per_buf, dsn="C:/Users/foeke/OneDrive/Documenten/april onwards/2022/Initial dataset/ForModelling/LocalModels/TrafficLocal", layer='traffic_NO2_Local', driver = "ESRI Shapefile")
